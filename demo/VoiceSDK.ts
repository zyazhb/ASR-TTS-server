...
export class VoiceClient {
   ...
   */
  async startASR(onMessage: (text: string, isFinal: boolean) => void): Promise<void> {
    ...
    const downsampleFactor = Math.max(1, Math.round(nativeRate / ASR_SAMPLE_RATE));
  ...
  stopASR(): void {
    ...
    if (this.asrStream) {
      this.asrStream.getTracks().forEach((t) => { t.stop(); });
      this.asrStream = null;
    }
    if (this.asrWs) {
      this.asrWs.close();
